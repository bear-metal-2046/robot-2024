...
//        }
    ...
    private void processVisionUpdate(PhotonPipelineResult result) {
        ...
        if (updateNotFlat || updateNotInField) {
            return;
        } else if (updateTooAmbiguous) {
            saveSnapshot(lastUpdateTime);
        }
    ...
    public void saveSnapshot(double timeStampSeconds) {
        if (Math.abs(lastSnapshotTime - timeStampSeconds) > VisionConstants.MIN_SNAPSHOT_DELAY) {
            logger.info("Camera snapshot taken at " + timeStampSeconds);
//            camera.takeInputSnapshot();
//            camera.takeOutputSnapshot();
            lastSnapshotTime = timeStampSeconds;
        }
    }
